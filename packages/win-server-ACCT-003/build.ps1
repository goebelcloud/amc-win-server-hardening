<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-003).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Minimum password age" (win-server-ACCT-003).
  Version: 1.0.0
#>
...
$settingTitle = "Minimum password age"
$settingPath  = "Account Policies\Password Policy"
$settingSuggestedValue = "1 day"



# Detailed build steps for win-server-ACCT-003 (Minimum password age):
...
#   2) Compile the DSC configuration 'ACCT_003_Minimum_password_age' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-003.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_003_Minimum_password_age -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_003_Minimum_password_age") "localhost.mof")
)
