...
Expected:
- `output/mof/SECO-001/localhost.mof`
- `output/zip/SECO-001/win-server-SECO-001.zip`
...
Expected:
- `output/policy/SECO-001/deployIfNotExists.json`
- `output/policy/SECO-001/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-001` exists.
- `properties.guestConfiguration.name = win-server-SECO-001`
...
Expected:
- recent entries for `win-server-SECO-001`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa' | Select-Object LimitBlankPasswordUse
```

Expected target value: LimitBlankPasswordUse = 1 (Enabled)
