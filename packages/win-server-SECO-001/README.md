...
- Registry: `HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\LimitBlankPasswordUse` (DWORD) = `1`
...
3. Open **Accounts: Limit local account use of blank passwords to console logon only** and confirm it is set to **Enabled**.
...
```
Expected: `LimitBlankPasswordUse` (DWord) = `1`.
...
## DSC Configuration
- Configuration name: `SECO_001_Accounts_Limit_local_account_use_of_blank_passwords`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-SECO-001.zip" `
