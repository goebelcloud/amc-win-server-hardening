<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-001).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Accounts: Limit local account use of blank passwords to console logon only" (win-server-SECO-001).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-001 (Accounts: Limit local account use of blank passwords to console logon only):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-001.zip
