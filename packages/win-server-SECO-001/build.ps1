<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Accounts: Limit local account use of blank passwords to console logon only" (win-server-SECO-001).
  Version: 1.0.0
#>
...
# Detailed build steps for win-server-SECO-001 (Accounts: Limit local account use of blank passwords to console logon only):
...
#   2) Compile the DSC configuration 'SECO_001_Accounts_Limit_local_account_use_of_blank_passwords' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_001_Accounts_Limit_local_account_use_of_blank_passwords -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_001_Accounts_Limit_local_account_use_of_blank_passwords") "localhost.mof")
)
