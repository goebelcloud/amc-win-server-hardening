<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-017).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Shutdown: Allow system to be shut down without having to log on" (win-server-SECO-017).
  Version: 1.0.0
#>
...
$settingTitle = "Shutdown: Allow system to be shut down without having to log on"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Disabled"



# Detailed build steps for win-server-SECO-017 (Shutdown: Allow system to be shut down without having to log on):
...
#   2) Compile the DSC configuration 'SECO_017_Shutdown_Allow_system_to_be_shut_down_without_havin' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-017.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_017_Shutdown_Allow_system_to_be_shut_down_without_havin -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_017_Shutdown_Allow_system_to_be_shut_down_without_havin") "localhost.mof")
)
