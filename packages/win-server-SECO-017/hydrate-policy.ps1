<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-017).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Shutdown: Allow system to be shut down without having to log on" (win-server-SECO-017).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-017 (Shutdown: Allow system to be shut down without having to log on):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-017.zip
