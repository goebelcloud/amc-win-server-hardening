...
- Registry: `HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\Real-Time Protection\DisableRealtimeMonitoring` (DWORD) = `0`
...
3. Open **Real-time protection** and confirm it is set to **Enabled (do not disable real-time monitoring)**.
...
```
Expected: `DisableRealtimeMonitoring` (DWord) = `0`.
...
## DSC Configuration
- Configuration name: `DEF_001_Real_time_protection`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-DEF-001.zip" `
