<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-DEF-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Real-time protection" (win-server-DEF-001).
  Version: 1.0.0
#>
...
$settingTitle = "Real-time protection"
$settingPath  = "Windows Components\Microsoft Defender Antivirus"
$settingSuggestedValue = "Enabled (do not disable real-time monitoring)"



# Detailed build steps for win-server-DEF-001 (Real-time protection):
...
#   2) Compile the DSC configuration 'DEF_001_Real_time_protection' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-DEF-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& DEF_001_Real_time_protection -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "DEF_001_Real_time_protection") "localhost.mof")
)
