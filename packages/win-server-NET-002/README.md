...
- **Setting path:** `Administrative Templates\Windows Components\AutoPlay Policies`
- **Setting:** `Turn off AutoPlay / AutoRun`
- **Suggested value:** `Enabled (All drives)`
- **Impact:** `Low`

## What this package changes
- Registry: `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\Explorer\NoDriveTypeAutoRun` (DWORD) = `255`

## How to verify the setting is applied (built-in OS tools)

### GUI verification
1. Press **Win+R**, run `gpedit.msc` (Local Group Policy Editor).
2. Navigate to: **Administrative Templates > Windows Components > AutoPlay Policies**.
3. Open **Turn off AutoPlay / AutoRun** and confirm it is set to **Enabled (All drives)**.
...
```
Expected: `NoDriveTypeAutoRun` (DWord) = `255`.
...
## DSC Configuration
- Configuration name: `NET_002_Turn_off_AutoPlay_AutoRun`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-NET-002.zip" `
