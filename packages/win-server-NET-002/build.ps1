<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-NET-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Turn off AutoPlay / AutoRun" (win-server-NET-002).
  Version: 1.0.0
#>
...
$settingTitle = "Turn off AutoPlay / AutoRun"
$settingPath  = "Administrative Templates\Windows Components\AutoPlay Policies"
$settingSuggestedValue = "Enabled (All drives)"



# Detailed build steps for win-server-NET-002 (Turn off AutoPlay / AutoRun):
...
#   2) Compile the DSC configuration 'NET_002_Turn_off_AutoPlay_AutoRun' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-NET-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& NET_002_Turn_off_AutoPlay_AutoRun -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "NET_002_Turn_off_AutoPlay_AutoRun") "localhost.mof")
)
