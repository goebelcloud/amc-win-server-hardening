<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-NET-002).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Turn off AutoPlay / AutoRun" (win-server-NET-002).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-NET-002 (Turn off AutoPlay / AutoRun):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-NET-002.zip
