<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-006).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Store passwords using reversible encryption" (win-server-ACCT-006).
  Version: 1.0.0
#>
...
$settingTitle = "Store passwords using reversible encryption"
$settingPath  = "Account Policies\Password Policy"
$settingSuggestedValue = "Disabled"



# Detailed build steps for win-server-ACCT-006 (Store passwords using reversible encryption):
...
#   2) Compile the DSC configuration 'ACCT_006_Store_passwords_using_reversible_encryption' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-006.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_006_Store_passwords_using_reversible_encryption -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_006_Store_passwords_using_reversible_encryption") "localhost.mof")
)
