...
Expected:
- `output/mof/ACCT-006/localhost.mof`
- `output/zip/ACCT-006/win-server-ACCT-006.zip`
...
Expected:
- `output/policy/ACCT-006/deployIfNotExists.json`
- `output/policy/ACCT-006/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-006` exists.
- `properties.guestConfiguration.name = win-server-ACCT-006`
...
Expected:
- recent entries for `win-server-ACCT-006`
...
Select-String -Path $tmp -Pattern '^ClearTextPassword\s*=\s*0$'
```

Expected target value: ClearTextPassword = 0 (Disabled)
