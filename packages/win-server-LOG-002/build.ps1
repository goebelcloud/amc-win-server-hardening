<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-LOG-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Increase Security log maximum size" (win-server-LOG-002).
  Version: 1.0.0
#>
...
$settingTitle = "Increase Security log maximum size"
$settingPath  = "Event Logs"
$settingSuggestedValue = ">= 196,608 KB (192 MB) (tune for environment)"



# Detailed build steps for win-server-LOG-002 (Increase Security log maximum size):
...
#   2) Compile the DSC configuration 'LOG_002_Increase_Security_log_maximum_size' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-LOG-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& LOG_002_Increase_Security_log_maximum_size -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "LOG_002_Increase_Security_log_maximum_size") "localhost.mof")
)
