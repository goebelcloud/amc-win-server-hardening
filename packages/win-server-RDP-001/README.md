...
- Registry: `HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp\UserAuthentication` (DWORD) = `1`
...
3. Open **Require user authentication for remote connections by using Network Level Authentication** and confirm it is set to **Enabled**.
...
```
Expected: `UserAuthentication` (DWord) = `1`.
...
## DSC Configuration
- Configuration name: `RDP_001_Require_user_authentication_for_remote_connections_b`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-RDP-001.zip" `
