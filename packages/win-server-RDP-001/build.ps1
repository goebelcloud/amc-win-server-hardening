<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-RDP-001).
...
.NOTES
  File: build.ps1
  Package: win-server-RDP-001 - Require user authentication for remote connections by using Network Level Authentication
  Purpose: Builds the Machine Configuration artifacts for the setting "Require user authentication for remote connections by using Network Level Authentication" (win-server-RDP-001).
  Version: 1.0.0
#>
...
$settingTitle = "Require user authentication for remote connections by using Network Level Authentication"
$settingPath  = "Computer Configuration\Administrative Templates\Windows Components\Remote Desktop Services\Remote Desktop Session Host\Security"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-RDP-001 (Require user authentication for remote connections by using Network Level Authentication):
...
#   2) Compile the DSC configuration 'RDP_001_Require_user_authentication_for_remote_connections_b' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-RDP-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& RDP_001_Require_user_authentication_for_remote_connections_b -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "RDP_001_Require_user_authentication_for_remote_connections_b") "localhost.mof")
)
