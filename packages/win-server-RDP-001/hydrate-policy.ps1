<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-RDP-001).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Require user authentication for remote connections by using Network Level Authentication" (win-server-RDP-001).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-RDP-001 (Require user authentication for remote connections by using Network Level Authentication):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-RDP-001.zip
