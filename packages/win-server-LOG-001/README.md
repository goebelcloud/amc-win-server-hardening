...
- **Impact:** `Low/Medium (log volume)`

## What this package changes
- Registry: `HKLM:\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging\EnableScriptBlockLogging` (DWORD) = `1`

## How to verify the setting is applied (built-in OS tools)

### GUI verification
1. Press **Win+R**, run `gpedit.msc` (Local Group Policy Editor).
2. Navigate to: **Windows Components > Windows PowerShell**.
3. Open **PowerShell Script Block Logging** and confirm it is set to **Enabled**.
...
```
Expected: `EnableScriptBlockLogging` (DWord) = `1`.
...
## DSC Configuration
- Configuration name: `LOG_001_PowerShell_Script_Block_Logging`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-LOG-001.zip" `
