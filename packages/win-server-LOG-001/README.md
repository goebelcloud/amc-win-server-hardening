...
- **Setting path:** `Windows Components\Windows PowerShell`
...
Expected:
- `output/mof/LOG-001/localhost.mof`
- `output/zip/LOG-001/win-server-LOG-001.zip`
...
Expected:
- `output/policy/LOG-001/deployIfNotExists.json`
- `output/policy/LOG-001/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-LOG-001` exists.
- `properties.guestConfiguration.name = win-server-LOG-001`
...
Expected:
- recent entries for `win-server-LOG-001`
- no recurring persistent errors

#### Target setting
GUI: open `gpedit.msc` and navigate to `Windows Components > Windows PowerShell`.

```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\PowerShell\ScriptBlockLogging' | Select-Object EnableScriptBlockLogging
```

Expected target value: EnableScriptBlockLogging = 1 (Enabled)
