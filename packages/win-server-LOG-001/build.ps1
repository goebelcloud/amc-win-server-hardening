<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-LOG-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "PowerShell Script Block Logging" (win-server-LOG-001).
  Version: 1.0.0
#>
...
$settingTitle = "PowerShell Script Block Logging"
$settingPath  = "Windows Components\Windows PowerShell"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-LOG-001 (PowerShell Script Block Logging):
...
#   2) Compile the DSC configuration 'LOG_001_PowerShell_Script_Block_Logging' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-LOG-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& LOG_001_PowerShell_Script_Block_Logging -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "LOG_001_PowerShell_Script_Block_Logging") "localhost.mof")
)
