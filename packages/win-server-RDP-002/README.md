...
- **Setting path:** `...\Remote Desktop Session Host\Security`
- **Setting:** `Set client connection encryption level`
- **Suggested value:** `High`
- **Impact:** `Low`

## What this package changes
- Registry: `HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp\MinEncryptionLevel` (DWORD) = `3`

## How to verify the setting is applied (built-in OS tools)

### GUI verification
1. Press **Win+R**, run `regedit.exe`.
2. Verify the setting **Set client connection encryption level** according to the registry/command checks below.
...
```
Expected: `MinEncryptionLevel` (DWord) = `3`.
...
## DSC Configuration
- Configuration name: `RDP_002_Set_client_connection_encryption_level`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-RDP-002.zip" `
