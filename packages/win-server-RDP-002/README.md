...
Expected:
- `output/mof/RDP-002/localhost.mof`
- `output/zip/RDP-002/win-server-RDP-002.zip`
...
Expected:
- `output/policy/RDP-002/deployIfNotExists.json`
- `output/policy/RDP-002/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-RDP-002` exists.
- `properties.guestConfiguration.name = win-server-RDP-002`
...
Expected:
- recent entries for `win-server-RDP-002`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Terminal Server\WinStations\RDP-Tcp' | Select-Object MinEncryptionLevel
```

Expected target value: MinEncryptionLevel = 3 (High)
