<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-RDP-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Set client connection encryption level" (win-server-RDP-002).
  Version: 1.0.0
#>
...
$settingTitle = "Set client connection encryption level"
$settingPath  = "...\Remote Desktop Session Host\Security"
$settingSuggestedValue = "High"



# Detailed build steps for win-server-RDP-002 (Set client connection encryption level):
...
#   2) Compile the DSC configuration 'RDP_002_Set_client_connection_encryption_level' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-RDP-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& RDP_002_Set_client_connection_encryption_level -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "RDP_002_Set_client_connection_encryption_level") "localhost.mof")
)
