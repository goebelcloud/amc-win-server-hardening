<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Accounts: Guest account status" (win-server-SECO-002).
  Version: 1.0.0
#>
...
# Detailed build steps for win-server-SECO-002 (Accounts: Guest account status):
...
#   2) Compile the DSC configuration 'SECO_002_Accounts_Guest_account_status' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_002_Accounts_Guest_account_status -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_002_Accounts_Guest_account_status") "localhost.mof")
)
