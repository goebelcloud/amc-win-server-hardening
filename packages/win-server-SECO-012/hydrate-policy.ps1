<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-012).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Microsoft network client: Digitally sign communications (always)" (win-server-SECO-012).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-012 (Microsoft network client: Digitally sign communications (always)):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-012.zip
