<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-012).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Microsoft network client: Digitally sign communications (always)" (win-server-SECO-012).
  Version: 1.0.0
#>
...
$settingTitle = "Microsoft network client: Digitally sign communications (always)"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Enabled (SMB signing required for outbound SMB client traffic)"



# Detailed build steps for win-server-SECO-012 (Microsoft network client: Digitally sign communications (always)):
...
#   2) Compile the DSC configuration 'SECO_012_Microsoft_network_client_Digitally_sign_communicati' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-012.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_012_Microsoft_network_client_Digitally_sign_communicati -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_012_Microsoft_network_client_Digitally_sign_communicati") "localhost.mof")
)
