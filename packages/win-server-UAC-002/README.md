...
Expected:
- `output/mof/UAC-002/localhost.mof`
- `output/zip/UAC-002/win-server-UAC-002.zip`
...
Expected:
- `output/policy/UAC-002/deployIfNotExists.json`
- `output/policy/UAC-002/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-UAC-002` exists.
- `properties.guestConfiguration.name = win-server-UAC-002`
...
Expected:
- recent entries for `win-server-UAC-002`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' | Select-Object PromptOnSecureDesktop
```

Expected target value: PromptOnSecureDesktop = 1 (Enabled)
