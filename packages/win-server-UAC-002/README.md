...
- Registry: `HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System\PromptOnSecureDesktop` (DWORD) = `1`
...
3. Open **User Account Control: Switch to the secure desktop when prompting for elevation** and confirm it is set to **Enabled**.
...
```
Expected: `PromptOnSecureDesktop` (DWord) = `1`.
...
## DSC Configuration
- Configuration name: `UAC_002_User_Account_Control_Switch_to_the_secure_desktop_wh`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-UAC-002.zip" `
