<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-UAC-002).
...
.NOTES
  File: build.ps1
  Package: win-server-UAC-002 - User Account Control: Switch to the secure desktop when prompting for elevation
  Purpose: Builds the Machine Configuration artifacts for the setting "User Account Control: Switch to the secure desktop when prompting for elevation" (win-server-UAC-002).
  Version: 1.0.0
#>
...
$settingTitle = "User Account Control: Switch to the secure desktop when prompting for elevation"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-UAC-002 (User Account Control: Switch to the secure desktop when prompting for elevation):
...
#   2) Compile the DSC configuration 'UAC_002_User_Account_Control_Switch_to_the_secure_desktop_wh' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-UAC-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& UAC_002_User_Account_Control_Switch_to_the_secure_desktop_wh -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "UAC_002_User_Account_Control_Switch_to_the_secure_desktop_wh") "localhost.mof")
)
