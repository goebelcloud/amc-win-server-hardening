<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-UAC-002).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "User Account Control: Switch to the secure desktop when prompting for elevation" (win-server-UAC-002).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-UAC-002 (User Account Control: Switch to the secure desktop when prompting for elevation):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-UAC-002.zip
