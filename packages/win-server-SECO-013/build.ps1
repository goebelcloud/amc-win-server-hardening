<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-013).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Microsoft network server: Digitally sign communications (always)" (win-server-SECO-013).
  Version: 1.0.0
#>
...
$settingTitle = "Microsoft network server: Digitally sign communications (always)"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Enabled (SMB signing required for inbound SMB server traffic)"



# Detailed build steps for win-server-SECO-013 (Microsoft network server: Digitally sign communications (always)):
...
#   2) Compile the DSC configuration 'SECO_013_Microsoft_network_server_Digitally_sign_communicati' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-013.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_013_Microsoft_network_server_Digitally_sign_communicati -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_013_Microsoft_network_server_Digitally_sign_communicati") "localhost.mof")
)
