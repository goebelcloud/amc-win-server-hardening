# win-server-SECO-013 — Microsoft network client: Digitally sign communications (always)
...
Expected:
- `output/mof/SECO-013/localhost.mof`
- `output/zip/SECO-013/win-server-SECO-013.zip`
...
Expected:
- `output/policy/SECO-013/deployIfNotExists.json`
- `output/policy/SECO-013/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-013` exists.
- `properties.guestConfiguration.name = win-server-SECO-013`
...
Expected:
- recent entries for `win-server-SECO-013`
