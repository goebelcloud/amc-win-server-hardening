...
Expected:
- `output/mof/UAC-001/localhost.mof`
- `output/zip/UAC-001/win-server-UAC-001.zip`
...
Expected:
- `output/policy/UAC-001/deployIfNotExists.json`
- `output/policy/UAC-001/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-UAC-001` exists.
- `properties.guestConfiguration.name = win-server-UAC-001`
...
Expected:
- recent entries for `win-server-UAC-001`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' | Select-Object EnableLUA
```

Expected target value: EnableLUA = 1 (Enabled)
