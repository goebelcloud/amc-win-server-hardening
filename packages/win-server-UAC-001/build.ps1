<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-UAC-001).
...
.NOTES
  File: build.ps1
  Package: win-server-UAC-001 - User Account Control: Run all administrators in Admin Approval Mode
  Purpose: Builds the Machine Configuration artifacts for the setting "User Account Control: Run all administrators in Admin Approval Mode" (win-server-UAC-001).
  Version: 1.0.0
#>
...
$settingTitle = "User Account Control: Run all administrators in Admin Approval Mode"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-UAC-001 (User Account Control: Run all administrators in Admin Approval Mode):
...
#   2) Compile the DSC configuration 'UAC_001_User_Account_Control_Run_all_administrators_in_Admin' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-UAC-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& UAC_001_User_Account_Control_Run_all_administrators_in_Admin -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "UAC_001_User_Account_Control_Run_all_administrators_in_Admin") "localhost.mof")
)
