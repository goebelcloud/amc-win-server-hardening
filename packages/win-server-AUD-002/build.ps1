<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-AUD-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Audit Logon/Logoff (Success/Failure)" (win-server-AUD-002).
  Version: 1.0.0
#>
...
$settingTitle = "Audit Logon/Logoff (Success/Failure)"
$settingPath  = "Advanced Audit Policy Configuration"
$settingSuggestedValue = "Enable Success and Failure"



# Detailed build steps for win-server-AUD-002 (Audit Logon/Logoff (Success/Failure)):
...
#   2) Compile the DSC configuration 'AUD_002_Audit_Logon_Logoff_Success_Failure' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-AUD-002.zip' using New-GuestConfigurationPackage.
