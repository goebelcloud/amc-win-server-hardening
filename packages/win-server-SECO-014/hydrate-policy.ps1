<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-014).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Interactive logon: Do not display last user name" (win-server-SECO-014).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-014 (Interactive logon: Do not display last user name):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-014.zip
