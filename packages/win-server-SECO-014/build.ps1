<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-014).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Interactive logon: Do not display last user name" (win-server-SECO-014).
  Version: 1.0.0
#>
...
$settingTitle = "Interactive logon: Do not display last user name"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-SECO-014 (Interactive logon: Do not display last user name):
...
#   2) Compile the DSC configuration 'SECO_014_Interactive_logon_Do_not_display_last_user_name' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-014.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_014_Interactive_logon_Do_not_display_last_user_name -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_014_Interactive_logon_Do_not_display_last_user_name") "localhost.mof")
)
