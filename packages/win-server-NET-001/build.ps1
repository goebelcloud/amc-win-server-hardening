<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-NET-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Disable SMBv1 (Client and Server)" (win-server-NET-001).
  Version: 1.0.0
#>
...
$settingTitle = "Disable SMBv1 (Client and Server)"
$settingPath  = "SMB"
$settingSuggestedValue = "Disable/remove SMBv1"



# Detailed build steps for win-server-NET-001 (Disable SMBv1 (Client and Server)):
...
#   2) Compile the DSC configuration 'NET_001_Disable_SMBv1_Client_and_Server' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-NET-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& NET_001_Disable_SMBv1_Client_and_Server -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "NET_001_Disable_SMBv1_Client_and_Server") "localhost.mof")
)
