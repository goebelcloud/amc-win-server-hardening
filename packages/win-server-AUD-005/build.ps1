<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-AUD-005).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Audit Privilege Use (Failure)" (win-server-AUD-005).
  Version: 1.0.0
#>
...
$settingTitle = "Audit Privilege Use (Failure)"
$settingPath  = "Advanced Audit Policy Configuration"
$settingSuggestedValue = "Enable Failure (and Success if required)"



# Detailed build steps for win-server-AUD-005 (Audit Privilege Use (Failure)):
...
#   2) Compile the DSC configuration 'AUD_005_Audit_Privilege_Use_Failure' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-AUD-005.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& AUD_005_Audit_Privilege_Use_Failure -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "AUD_005_Audit_Privilege_Use_Failure") "localhost.mof")
)
