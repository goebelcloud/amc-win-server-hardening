<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-DEF-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Cloud-delivered protection" (win-server-DEF-002).
  Version: 1.0.0
#>
...
$settingTitle = "Cloud-delivered protection"
$settingPath  = "Windows Components\Microsoft Defender Antivirus"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-DEF-002 (Cloud-delivered protection):
...
#   2) Compile the DSC configuration 'DEF_002_Cloud_delivered_protection' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-DEF-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& DEF_002_Cloud_delivered_protection -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "DEF_002_Cloud_delivered_protection") "localhost.mof")
)
