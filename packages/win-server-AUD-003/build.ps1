<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-AUD-003).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Audit Account Management (Success/Failure)" (win-server-AUD-003).
  Version: 1.0.0
#>
...
$settingTitle = "Audit Account Management (Success/Failure)"
$settingPath  = "Advanced Audit Policy Configuration"
$settingSuggestedValue = "Enable Success and Failure"



# Detailed build steps for win-server-AUD-003 (Audit Account Management (Success/Failure)):
...
#   2) Compile the DSC configuration 'AUD_003_Audit_Account_Management_Success_Failure' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-AUD-003.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& AUD_003_Audit_Account_Management_Success_Failure -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "AUD_003_Audit_Account_Management_Success_Failure") "localhost.mof")
)
