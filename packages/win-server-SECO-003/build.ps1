<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-003).
...
.NOTES
  File: build.ps1
  Package: win-server-SECO-003 - Audit: Built-in Administrator (RID 500) is not named 'Administrator'
  Purpose: Builds Machine Configuration artifacts for win-server-SECO-003 (audit RID-500 Administrator name).
  Version: 1.0.0
#>
...
$settingTitle = "Audit: Built-in Administrator (RID 500) is not named 'Administrator'"
$settingPath  = "Security Options > Accounts"
$settingSuggestedValue = "Compliant if Name != 'Administrator' (audit only; no remediation)"



# Detailed build steps for win-server-SECO-003 (Audit: Built-in Administrator (RID 500) is not named 'Administrator'):
...
#   2) Compile the DSC configuration 'SECO_003_AuditAdminNotDefaultName' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-003.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_003_AuditAdminNotDefaultName -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_003_AuditAdminNotDefaultName") "localhost.mof")
)
