...
Expected:
- `output/mof/SECO-003/localhost.mof`
- `output/zip/SECO-003/win-server-SECO-003.zip`
...
Expected:
- `output/policy/SECO-003/deployIfNotExists.json`
- `output/policy/SECO-003/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-003` exists.
- `properties.guestConfiguration.name = win-server-SECO-003`
...
Expected:
- recent entries for `win-server-SECO-003`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa' | Select-Object RestrictAnonymousSAM
```

Expected target value: RestrictAnonymousSAM = 1 (Enabled)
