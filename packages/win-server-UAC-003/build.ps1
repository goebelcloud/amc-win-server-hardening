<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-UAC-003).
...
.NOTES
  File: build.ps1
  Package: win-server-UAC-003 - User Account Control: Behavior of the elevation prompt for administrators in Admin Approval Mode
  Purpose: Builds the Machine Configuration artifacts for the setting "User Account Control: Behavior of the elevation prompt for administrators in Admin Approval Mode" (win-server-UAC-003).
  Version: 1.0.0
#>
...
$settingTitle = "User Account Control: Behavior of the elevation prompt for administrators in Admin Approval Mode"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Prompt for consent on the secure desktop"



# Detailed build steps for win-server-UAC-003 (User Account Control: Behavior of the elevation prompt for administrators in Admin Approval Mode):
...
#   2) Compile the DSC configuration 'UAC_003_User_Account_Control_Behavior_of_the_elevation_promp' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-UAC-003.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& UAC_003_User_Account_Control_Behavior_of_the_elevation_promp -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "UAC_003_User_Account_Control_Behavior_of_the_elevation_promp") "localhost.mof")
)
