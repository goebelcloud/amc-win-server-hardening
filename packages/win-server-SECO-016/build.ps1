<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-016).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Interactive logon: Machine inactivity limit" (win-server-SECO-016).
  Version: 1.0.0
#>
...
$settingTitle = "Interactive logon: Machine inactivity limit"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "900 seconds (15 minutes)"



# Detailed build steps for win-server-SECO-016 (Interactive logon: Machine inactivity limit):
...
#   2) Compile the DSC configuration 'SECO_016_Interactive_logon_Machine_inactivity_limit' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-016.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_016_Interactive_logon_Machine_inactivity_limit -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_016_Interactive_logon_Machine_inactivity_limit") "localhost.mof")
)
