<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-RDP-003).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Do not allow passwords to be saved" (win-server-RDP-003).
  Version: 1.0.0
#>
...
$settingTitle = "Do not allow passwords to be saved"
$settingPath  = "...\Remote Desktop Session Host\Security"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-RDP-003 (Do not allow passwords to be saved):
...
#   2) Compile the DSC configuration 'RDP_003_Do_not_allow_passwords_to_be_saved' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-RDP-003.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& RDP_003_Do_not_allow_passwords_to_be_saved -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "RDP_003_Do_not_allow_passwords_to_be_saved") "localhost.mof")
)
