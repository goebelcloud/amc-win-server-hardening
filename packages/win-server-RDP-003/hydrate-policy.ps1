<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-RDP-003).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Do not allow passwords to be saved" (win-server-RDP-003).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-RDP-003 (Do not allow passwords to be saved):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-RDP-003.zip
