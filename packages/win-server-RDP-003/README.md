...
- Registry: `HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\Terminal Services\DisablePasswordSaving` (DWORD) = `1`

## How to verify the setting is applied (built-in OS tools)

### GUI verification
1. Press **Win+R**, run `regedit.exe`.
2. Verify the setting **Do not allow passwords to be saved** according to the registry/command checks below.
...
```
Expected: `DisablePasswordSaving` (DWord) = `1`.
...
## DSC Configuration
- Configuration name: `RDP_003_Do_not_allow_passwords_to_be_saved`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-RDP-003.zip" `
