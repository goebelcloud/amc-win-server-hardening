...
- Registry: `HKLM:\SOFTWARE\Policies\Microsoft\Windows\WinRM\Service\AllowUnencryptedTraffic` (DWORD) = `0`
...
3. Open **Allow unencrypted traffic** and confirm it is set to **Disabled**.
...
```
Expected: `AllowUnencryptedTraffic` (DWord) = `0`.
...
## DSC Configuration
- Configuration name: `WINRM_001_Allow_unencrypted_traffic`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-WINRM-001.zip" `
