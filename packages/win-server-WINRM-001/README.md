...
Expected:
- `output/mof/WINRM-001/localhost.mof`
- `output/zip/WINRM-001/win-server-WINRM-001.zip`
...
Expected:
- `output/policy/WINRM-001/deployIfNotExists.json`
- `output/policy/WINRM-001/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-WINRM-001` exists.
- `properties.guestConfiguration.name = win-server-WINRM-001`
...
Expected:
- recent entries for `win-server-WINRM-001`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\WinRM\Service' | Select-Object AllowUnencryptedTraffic
```

Expected target value: AllowUnencryptedTraffic = 0 (Disabled)
