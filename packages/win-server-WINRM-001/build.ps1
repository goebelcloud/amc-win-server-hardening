<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-WINRM-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Allow unencrypted traffic" (win-server-WINRM-001).
  Version: 1.0.0
#>
...
$settingTitle = "Allow unencrypted traffic"
$settingPath  = "Windows Components\Windows Remote Management (WinRM)\WinRM Service"
$settingSuggestedValue = "Disabled"



# Detailed build steps for win-server-WINRM-001 (Allow unencrypted traffic):
...
#   2) Compile the DSC configuration 'WINRM_001_Allow_unencrypted_traffic' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-WINRM-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& WINRM_001_Allow_unencrypted_traffic -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "WINRM_001_Allow_unencrypted_traffic") "localhost.mof")
)
