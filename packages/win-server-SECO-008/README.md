# win-server-SECO-008 — Interactive logon: Do not display last user name
...
Expected:
- `output/mof/SECO-008/localhost.mof`
- `output/zip/SECO-008/win-server-SECO-008.zip`
...
Expected:
- `output/policy/SECO-008/deployIfNotExists.json`
- `output/policy/SECO-008/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-008` exists.
- `properties.guestConfiguration.name = win-server-SECO-008`
...
Expected:
- recent entries for `win-server-SECO-008`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System' | Select-Object DontDisplayLastUserName
```

Expected target value: DontDisplayLastUserName = 1 (Enabled)
