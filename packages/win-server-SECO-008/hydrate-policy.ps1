<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-008).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Network security: Do not store LAN Manager hash value on next password change" (win-server-SECO-008).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-008 (Network security: Do not store LAN Manager hash value on next password change):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-008.zip
