...
- Local Security Policy export (`secedit`) key `PasswordComplexity` = `1` in `[System Access]`

## How to evaluate the setting (built-in OS tools)
**Export and inspect local security policy**
```cmd
secedit /export /cfg %TEMP%\win-server-ACCT-005-export.inf
findstr /i "^PasswordComplexity" %TEMP%\win-server-ACCT-005-export.inf
```

## Manual remediation (built-in OS tools)
Apply via security template:
```cmd
secedit /configure /db %TEMP%\secedit.sdb /cfg <your.inf> /areas SECURITYPOLICY
```
...
## DSC Configuration
- Configuration name: `ACCT_005_Password_must_meet_complexity_requirements`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-ACCT-005.zip" `
