...
Expected:
- `output/mof/ACCT-005/localhost.mof`
- `output/zip/ACCT-005/win-server-ACCT-005.zip`
...
Expected:
- `output/policy/ACCT-005/deployIfNotExists.json`
- `output/policy/ACCT-005/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-005` exists.
- `properties.guestConfiguration.name = win-server-ACCT-005`
...
Expected:
- recent entries for `win-server-ACCT-005`
...
Select-String -Path $tmp -Pattern '^PasswordComplexity\s*=\s*1$'
```

Expected target value: PasswordComplexity = 1 (Enabled)
