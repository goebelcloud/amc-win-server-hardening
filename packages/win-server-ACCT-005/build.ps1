<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-005).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Password must meet complexity requirements" (win-server-ACCT-005).
  Version: 1.0.0
#>
...
# Detailed build steps for win-server-ACCT-005 (Password must meet complexity requirements):
...
#   2) Compile the DSC configuration 'ACCT_005_Password_must_meet_complexity_requirements' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-005.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_005_Password_must_meet_complexity_requirements -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_005_Password_must_meet_complexity_requirements") "localhost.mof")
)
