<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-FW-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Enable firewall logging" (win-server-FW-002).
  Version: 1.0.0
#>
...
$settingTitle = "Enable firewall logging"
$settingPath  = "Windows Defender Firewall with Advanced Security"
$settingSuggestedValue = "Log dropped packets = Yes; Log successful connections = (Optional); Log size >= 16,384 KB"



# Detailed build steps for win-server-FW-002 (Enable firewall logging):
...
#   2) Compile the DSC configuration 'FW_002_Enable_firewall_logging' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-FW-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& FW_002_Enable_firewall_logging -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "FW_002_Enable_firewall_logging") "localhost.mof")
)
