...
Expected:
- `output/mof/SECO-005/localhost.mof`
- `output/zip/SECO-005/win-server-SECO-005.zip`
...
Expected:
- `output/policy/SECO-005/deployIfNotExists.json`
- `output/policy/SECO-005/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-005` exists.
- `properties.guestConfiguration.name = win-server-SECO-005`
...
Expected:
- recent entries for `win-server-SECO-005`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa' | Select-Object EveryoneIncludesAnonymous
```

Expected target value: EveryoneIncludesAnonymous = 0 (Disabled)
