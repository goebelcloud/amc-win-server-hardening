<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-015).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Interactive logon: Do not require CTRL+ALT+DEL" (win-server-SECO-015).
  Version: 1.0.0
#>
...
$settingTitle = "Interactive logon: Do not require CTRL+ALT+DEL"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Disabled (require CTRL+ALT+DEL)"



# Detailed build steps for win-server-SECO-015 (Interactive logon: Do not require CTRL+ALT+DEL):
...
#   2) Compile the DSC configuration 'SECO_015_Interactive_logon_Do_not_require_CTRL_ALT_DEL' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-015.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_015_Interactive_logon_Do_not_require_CTRL_ALT_DEL -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_015_Interactive_logon_Do_not_require_CTRL_ALT_DEL") "localhost.mof")
)
