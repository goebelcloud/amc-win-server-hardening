<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-015).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Interactive logon: Do not require CTRL+ALT+DEL" (win-server-SECO-015).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-015 (Interactive logon: Do not require CTRL+ALT+DEL):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-015.zip
