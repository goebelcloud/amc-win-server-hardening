...
- Registry: `HKLM:\SOFTWARE\Policies\Microsoft\Windows Defender\PUAProtection` (DWORD) = `1`
...
3. Open **Potentially Unwanted Application (PUA) protection** and confirm it is set to **Enabled (Block)**.

### Command-line verification
**Registry check (PowerShell / reg.exe)**
```cmd
reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows Defender" /v PUAProtection
```
Expected: `PUAProtection` (DWord) = `1`.
...
## DSC Configuration
- Configuration name: `DEF_003_Potentially_Unwanted_Application_PUA_protection`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-DEF-003.zip" `
