<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-DEF-003).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Potentially Unwanted Application (PUA) protection" (win-server-DEF-003).
  Version: 1.0.0
#>
...
$settingTitle = "Potentially Unwanted Application (PUA) protection"
$settingPath  = "Windows Components\Microsoft Defender Antivirus"
$settingSuggestedValue = "Enabled (Block)"



# Detailed build steps for win-server-DEF-003 (Potentially Unwanted Application (PUA) protection):
...
#   2) Compile the DSC configuration 'DEF_003_Potentially_Unwanted_Application_PUA_protection' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-DEF-003.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& DEF_003_Potentially_Unwanted_Application_PUA_protection -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "DEF_003_Potentially_Unwanted_Application_PUA_protection") "localhost.mof")
)
