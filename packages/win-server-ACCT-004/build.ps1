<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-004).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Minimum password length" (win-server-ACCT-004).
  Version: 1.0.0
#>
...
$settingTitle = "Minimum password length"
$settingPath  = "Account Policies\Password Policy"
$settingSuggestedValue = "14 characters"



# Detailed build steps for win-server-ACCT-004 (Minimum password length):
...
#   2) Compile the DSC configuration 'ACCT_004_Minimum_password_length' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-004.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_004_Minimum_password_length -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_004_Minimum_password_length") "localhost.mof")
)
