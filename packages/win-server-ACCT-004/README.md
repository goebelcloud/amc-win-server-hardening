...
Expected:
- `output/mof/ACCT-004/localhost.mof`
- `output/zip/ACCT-004/win-server-ACCT-004.zip`
...
Expected:
- `output/policy/ACCT-004/deployIfNotExists.json`
- `output/policy/ACCT-004/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-004` exists.
- `properties.guestConfiguration.name = win-server-ACCT-004`
...
Expected:
- recent entries for `win-server-ACCT-004`
...
Select-String -Path $tmp -Pattern '^MinimumPasswordLength\s*=\s*14$'
```

Expected target value: MinimumPasswordLength = 14 (14 characters)
