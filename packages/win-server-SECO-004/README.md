...
Expected:
- `output/mof/SECO-004/localhost.mof`
- `output/zip/SECO-004/win-server-SECO-004.zip`
...
Expected:
- `output/policy/SECO-004/deployIfNotExists.json`
- `output/policy/SECO-004/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-004` exists.
- `properties.guestConfiguration.name = win-server-SECO-004`
...
Expected:
- recent entries for `win-server-SECO-004`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa' | Select-Object RestrictAnonymous
```

Expected target value: RestrictAnonymous = 1 (Enabled)
