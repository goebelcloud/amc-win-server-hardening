<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-004).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Accounts: Rename guest account" (win-server-SECO-004).
  Version: 1.0.0
#>
...
$settingTitle = "Accounts: Rename guest account"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Rename to non-default (even if disabled)"



# Detailed build steps for win-server-SECO-004 (Accounts: Rename guest account):
...
#   2) Compile the DSC configuration 'SECO_004_Accounts_Rename_guest_account' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-004.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_004_Accounts_Rename_guest_account -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_004_Accounts_Rename_guest_account") "localhost.mof")
)
