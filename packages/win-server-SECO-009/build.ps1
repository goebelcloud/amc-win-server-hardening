<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-009).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Network security: LAN Manager authentication level" (win-server-SECO-009).
  Version: 1.0.0
#>
...
$settingTitle = "Network security: LAN Manager authentication level"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Send NTLMv2 response only. Refuse LM & NTLM"



# Detailed build steps for win-server-SECO-009 (Network security: LAN Manager authentication level):
...
#   2) Compile the DSC configuration 'SECO_009_Network_security_LAN_Manager_authentication_level' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-009.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_009_Network_security_LAN_Manager_authentication_level -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_009_Network_security_LAN_Manager_authentication_level") "localhost.mof")
)
