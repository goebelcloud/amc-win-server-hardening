<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-006).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Network access: Do not allow anonymous enumeration of SAM accounts and shares" (win-server-SECO-006).
  Version: 1.0.0
#>
...
$settingTitle = "Network access: Do not allow anonymous enumeration of SAM accounts and shares"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Enabled"



# Detailed build steps for win-server-SECO-006 (Network access: Do not allow anonymous enumeration of SAM accounts and shares):
...
#   2) Compile the DSC configuration 'SECO_006_Network_access_Do_not_allow_anonymous_enumeration_o' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-006.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_006_Network_access_Do_not_allow_anonymous_enumeration_o -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_006_Network_access_Do_not_allow_anonymous_enumeration_o") "localhost.mof")
)
