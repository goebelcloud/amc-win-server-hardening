<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-006).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Network access: Do not allow anonymous enumeration of SAM accounts and shares" (win-server-SECO-006).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-006 (Network access: Do not allow anonymous enumeration of SAM accounts and shares):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-006.zip
