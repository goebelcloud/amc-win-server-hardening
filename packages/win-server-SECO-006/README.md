...
Expected:
- `output/mof/SECO-006/localhost.mof`
- `output/zip/SECO-006/win-server-SECO-006.zip`
...
Expected:
- `output/policy/SECO-006/deployIfNotExists.json`
- `output/policy/SECO-006/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-SECO-006` exists.
- `properties.guestConfiguration.name = win-server-SECO-006`
...
Expected:
- recent entries for `win-server-SECO-006`
...
```powershell
Get-ItemProperty -Path 'Registry::HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Lsa' | Select-Object NoLMHash
```

Expected target value: NoLMHash = 1 (Enabled)
