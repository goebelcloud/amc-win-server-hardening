<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Enforce password history" (win-server-ACCT-001).
  Version: 1.0.0
#>
...
$settingTitle = "Enforce password history"
$settingPath  = "Account Policies\Password Policy"
$settingSuggestedValue = "24 passwords remembered"



# Detailed build steps for win-server-ACCT-001 (Enforce password history):
...
#   2) Compile the DSC configuration 'ACCT_001_Enforce_password_history' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_001_Enforce_password_history -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_001_Enforce_password_history") "localhost.mof")
)
