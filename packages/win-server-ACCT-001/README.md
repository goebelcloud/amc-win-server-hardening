...
Expected:
- `output/mof/ACCT-001/localhost.mof`
...
Expected:
- An assignment for `win-server-ACCT-001` exists.
- `properties.guestConfiguration.name = win-server-ACCT-001`
...
Expected:
- recent entries for `win-server-ACCT-001`
...
Select-String -Path $tmp -Pattern '^PasswordHistorySize\s*=\s*24$'
```

Expected target value: PasswordHistorySize = 24 (24 passwords remembered)
