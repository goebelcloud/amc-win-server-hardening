<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-010).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Network security: Minimum session security for NTLM SSP based (including secure RPC) clients" (win-server-SECO-010).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-010 (Network security: Minimum session security for NTLM SSP based (including secure RPC) clients):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-010.zip
