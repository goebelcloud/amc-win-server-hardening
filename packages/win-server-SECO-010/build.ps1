<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-010).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Network security: Minimum session security for NTLM SSP based (including secure RPC) clients" (win-server-SECO-010).
  Version: 1.0.0
#>
...
$settingTitle = "Network security: Minimum session security for NTLM SSP based (including secure RPC) clients"
...
# Detailed build steps for win-server-SECO-010 (Network security: Minimum session security for NTLM SSP based (including secure RPC) clients):
...
#   2) Compile the DSC configuration 'SECO_010_Network_security_Minimum_session_security_for_NTLM_' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-010.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_010_Network_security_Minimum_session_security_for_NTLM_ -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_010_Network_security_Minimum_session_security_for_NTLM_") "localhost.mof")
)
