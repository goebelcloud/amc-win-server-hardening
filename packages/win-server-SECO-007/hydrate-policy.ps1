<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-SECO-007).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Network access: Let Everyone permissions apply to anonymous users" (win-server-SECO-007).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-SECO-007 (Network access: Let Everyone permissions apply to anonymous users):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-SECO-007.zip
