<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-SECO-007).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Network access: Let Everyone permissions apply to anonymous users" (win-server-SECO-007).
  Version: 1.0.0
#>
...
$settingTitle = "Network access: Let Everyone permissions apply to anonymous users"
$settingPath  = "Local Policies\Security Options"
$settingSuggestedValue = "Disabled"



# Detailed build steps for win-server-SECO-007 (Network access: Let Everyone permissions apply to anonymous users):
...
#   2) Compile the DSC configuration 'SECO_007_Network_access_Let_Everyone_permissions_apply_to_an' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-SECO-007.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& SECO_007_Network_access_Let_Everyone_permissions_apply_to_an -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "SECO_007_Network_access_Let_Everyone_permissions_apply_to_an") "localhost.mof")
)
