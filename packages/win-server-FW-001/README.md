...
Expected:
- `output/mof/FW-001/localhost.mof`
- `output/zip/FW-001/win-server-FW-001.zip`
...
Expected:
- `output/policy/FW-001/deployIfNotExists.json`
- `output/policy/FW-001/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-FW-001` exists.
- `properties.guestConfiguration.name = win-server-FW-001`
...
Expected:
- recent entries for `win-server-FW-001`
- no recurring persistent errors

#### Target setting
GUI: open `wf.msc` and navigate to `Windows Defender Firewall with Advanced Security > Windows Defender Firewall Properties`.

```powershell
Get-NetFirewallProfile | Select-Object Name, Enabled, DefaultInboundAction
```

Expected target value: All profiles enabled; DefaultInboundAction = Block.
