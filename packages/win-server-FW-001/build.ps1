<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-FW-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Enable firewall for all profiles" (win-server-FW-001).
  Version: 1.0.0
#>
...
$settingTitle = "Enable firewall for all profiles"
$settingPath  = "Windows Defender Firewall with Advanced Security"
$settingSuggestedValue = "Enabled (Domain/Private/Public) + default inbound = Block"



# Detailed build steps for win-server-FW-001 (Enable firewall for all profiles):
...
#   2) Compile the DSC configuration 'FW_001_Enable_firewall_for_all_profiles' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-FW-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& FW_001_Enable_firewall_for_all_profiles -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "FW_001_Enable_firewall_for_all_profiles") "localhost.mof")
)
