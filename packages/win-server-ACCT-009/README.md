...
Expected:
- `output/mof/ACCT-009/localhost.mof`
- `output/zip/ACCT-009/win-server-ACCT-009.zip`
...
Expected:
- `output/policy/ACCT-009/deployIfNotExists.json`
- `output/policy/ACCT-009/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-009` exists.
- `properties.guestConfiguration.name = win-server-ACCT-009`
...
Expected:
- recent entries for `win-server-ACCT-009`
...
Select-String -Path $tmp -Pattern '^ResetLockoutCount\s*=\s*15$'
```

Expected target value: ResetLockoutCount = 15 (15 minutes)
