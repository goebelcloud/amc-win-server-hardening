...
- Registry: `HKLM:\SYSTEM\CurrentControlSet\Control\Lsa\RunAsPPL` (DWORD) = `1`

## How to verify the setting is applied (built-in OS tools)

### GUI verification
1. Press **Win+R**, run `regedit.exe`.
2. Verify the setting **Enable LSA protection (RunAsPPL)** according to the registry/command checks below.
...
## DSC Configuration
- Configuration name: `LSA_001_Enable_LSA_protection_RunAsPPL`
...
2. From this package directory, run:
   ...
   ./build.ps1 `
     ...
     -ContentUri "https://<storage>.blob.core.windows.net/<container>/win-server-LSA-001.zip" `
