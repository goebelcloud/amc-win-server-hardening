<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-LSA-001).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Enable LSA protection (RunAsPPL)" (win-server-LSA-001).
  Version: 1.0.0
#>
...
$settingTitle = "Enable LSA protection (RunAsPPL)"
$settingPath  = "System\Local Security Authority"
$settingSuggestedValue = "Enabled (Audit first if possible)"



# Detailed build steps for win-server-LSA-001 (Enable LSA protection (RunAsPPL)):
...
#   2) Compile the DSC configuration 'LSA_001_Enable_LSA_protection_RunAsPPL' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-LSA-001.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& LSA_001_Enable_LSA_protection_RunAsPPL -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "LSA_001_Enable_LSA_protection_RunAsPPL") "localhost.mof")
)
