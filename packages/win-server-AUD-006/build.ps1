<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-AUD-006).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Audit System (Success/Failure)" (win-server-AUD-006).
  Version: 1.0.0
#>
...
$settingTitle = "Audit System (Success/Failure)"
$settingPath  = "Advanced Audit Policy Configuration"
$settingSuggestedValue = "Enable Success and Failure"



# Detailed build steps for win-server-AUD-006 (Audit System (Success/Failure)):
...
#   2) Compile the DSC configuration 'AUD_006_Audit_System_Success_Failure' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-AUD-006.zip' using New-GuestConfigurationPackage.
...
#        - Advanced Audit Policy Configuration -> Audit System (Success/Failure) = Enable Success and Failure
...
# Compile MOF
& AUD_006_Audit_System_Success_Failure -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "AUD_006_Audit_System_Success_Failure") "localhost.mof")
)
