# win-server-AUD-006 — Audit System (Success/Failure)
...
Expected:
- `output/mof/AUD-006/localhost.mof`
- `output/zip/AUD-006/win-server-AUD-006.zip`
...
Expected:
- `output/policy/AUD-006/deployIfNotExists.json`
- `output/policy/AUD-006/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-AUD-006` exists.
- `properties.guestConfiguration.name = win-server-AUD-006`
...
Expected:
- recent entries for `win-server-AUD-006`
- no recurring persistent errors

#### Target setting
GUI: open `secpol.msc` and navigate to `Advanced Audit Policy Configuration`.

```powershell
auditpol /get /category:"System"
```

Expected target value: Current setting shows Success and Failure.
