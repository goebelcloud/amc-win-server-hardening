<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-AUD-004).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Audit Policy Change (Success/Failure)" (win-server-AUD-004).
  Version: 1.0.0
#>
...
$settingTitle = "Audit Policy Change (Success/Failure)"
$settingPath  = "Advanced Audit Policy Configuration"
$settingSuggestedValue = "Enable Success and Failure"



# Detailed build steps for win-server-AUD-004 (Audit Policy Change (Success/Failure)):
...
#   2) Compile the DSC configuration 'AUD_004_Audit_Policy_Change_Success_Failure' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-AUD-004.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& AUD_004_Audit_Policy_Change_Success_Failure -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "AUD_004_Audit_Policy_Change_Success_Failure") "localhost.mof")
)
