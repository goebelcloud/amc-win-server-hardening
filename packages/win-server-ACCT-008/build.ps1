<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-008).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Account lockout duration" (win-server-ACCT-008).
  Version: 1.0.0
#>
...
$settingTitle = "Account lockout duration"
$settingPath  = "Account Policies\Account Lockout Policy"
$settingSuggestedValue = "15 minutes"



# Detailed build steps for win-server-ACCT-008 (Account lockout duration):
...
#   2) Compile the DSC configuration 'ACCT_008_Account_lockout_duration' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-008.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_008_Account_lockout_duration -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_008_Account_lockout_duration") "localhost.mof")
)
