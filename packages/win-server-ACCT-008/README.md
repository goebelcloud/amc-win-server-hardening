...
Expected:
- `output/mof/ACCT-008/localhost.mof`
- `output/zip/ACCT-008/win-server-ACCT-008.zip`
...
Expected:
- `output/policy/ACCT-008/deployIfNotExists.json`
- `output/policy/ACCT-008/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-008` exists.
- `properties.guestConfiguration.name = win-server-ACCT-008`
...
Expected:
- recent entries for `win-server-ACCT-008`
...
Select-String -Path $tmp -Pattern '^LockoutDuration\s*=\s*15$'
```

Expected target value: LockoutDuration = 15 (15 minutes)
