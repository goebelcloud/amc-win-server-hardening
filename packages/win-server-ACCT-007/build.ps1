<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-007).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Account lockout threshold" (win-server-ACCT-007).
  Version: 1.0.0
#>
...
# Detailed build steps for win-server-ACCT-007 (Account lockout threshold):
...
#   2) Compile the DSC configuration 'ACCT_007_Account_lockout_threshold' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-007.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_007_Account_lockout_threshold -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_007_Account_lockout_threshold") "localhost.mof")
)
