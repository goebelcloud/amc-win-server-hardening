...
- **Setting path:** `Account Policies\Account Lockout Policy`
...
Expected:
- `output/mof/ACCT-007/localhost.mof`
- `output/zip/ACCT-007/win-server-ACCT-007.zip`
...
Expected:
- `output/policy/ACCT-007/deployIfNotExists.json`
- `output/policy/ACCT-007/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-007` exists.
- `properties.guestConfiguration.name = win-server-ACCT-007`
...
Expected:
- recent entries for `win-server-ACCT-007`
- no recurring persistent errors

#### Target setting
GUI: open `secpol.msc` and navigate to `Account Policies > Account Lockout Policy`.

```powershell
          $tmp = Join-Path $env:TEMP 'secpol.inf'
secedit /export /cfg $tmp | Out-Null
Select-String -Path $tmp -Pattern '^LockoutBadCount\s*=\s*10$'
```

Expected target value: LockoutBadCount = 10 (10 invalid logon attempts (WS2025 baseline uses 3; test before lowering))
