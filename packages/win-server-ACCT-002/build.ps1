<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-ACCT-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Maximum password age" (win-server-ACCT-002).
  Version: 1.0.0
#>
...
$settingTitle = "Maximum password age"
$settingPath  = "Account Policies\Password Policy"
$settingSuggestedValue = "60 days (or org standard 60–90)"



# Detailed build steps for win-server-ACCT-002 (Maximum password age):
...
#   2) Compile the DSC configuration 'ACCT_002_Maximum_password_age' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-ACCT-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& ACCT_002_Maximum_password_age -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "ACCT_002_Maximum_password_age") "localhost.mof")
)
