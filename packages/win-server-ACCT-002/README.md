...
Expected:
- `output/mof/ACCT-002/localhost.mof`
- `output/zip/ACCT-002/win-server-ACCT-002.zip`
...
Expected:
- `output/policy/ACCT-002/deployIfNotExists.json`
- `output/policy/ACCT-002/deployIfNotExists.enhanced.json`
...
Expected:
- An assignment for `win-server-ACCT-002` exists.
- `properties.guestConfiguration.name = win-server-ACCT-002`
...
Expected:
- recent entries for `win-server-ACCT-002`
...
Select-String -Path $tmp -Pattern '^MaximumPasswordAge\s*=\s*60$'
```

Expected target value: MaximumPasswordAge = 60 (60 days (or org standard 60–90))
