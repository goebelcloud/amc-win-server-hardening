<#
.SYNOPSIS
  Build this Machine Configuration package (win-server-WINRM-002).
...
.NOTES
  ...
  Purpose: Builds the Machine Configuration artifacts for the setting "Disallow Basic authentication" (win-server-WINRM-002).
  Version: 1.0.0
#>
...
$settingTitle = "Disallow Basic authentication"
$settingPath  = "Windows Components\Windows Remote Management (WinRM)\WinRM Service"
$settingSuggestedValue = "Basic = Disabled (use Kerberos/cert)"



# Detailed build steps for win-server-WINRM-002 (Disallow Basic authentication):
...
#   2) Compile the DSC configuration 'WINRM_002_Disallow_Basic_authentication' to produce a node MOF for localhost.
#      - Most common output: <MofOutputPath>\localhost.mof
#   3) Create the Machine Configuration package ZIP 'win-server-WINRM-002.zip' using New-GuestConfigurationPackage.
...
# Compile MOF
& WINRM_002_Disallow_Basic_authentication -OutputPath $mofOutputPath
...
$mofCandidatePaths = @(
  ...
  (Join-Path (Join-Path $mofOutputPath "WINRM_002_Disallow_Basic_authentication") "localhost.mof")
)
