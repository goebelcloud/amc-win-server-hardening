<#
.SYNOPSIS
  Hydrate policy JSON for this package (enhanced + non-enhanced) (win-server-WINRM-002).
...
.NOTES
  ...
  Purpose: Hydrates Azure Policy JSON templates (enhanced + non-enhanced) for the setting "Disallow Basic authentication" (win-server-WINRM-002).
  Version: 1.0.0
#>
...
# Detailed hydration steps for win-server-WINRM-002 (Disallow Basic authentication):
#   1) Locate the built package ZIP in the configured PackageZipOutputRoot and calculate SHA256 (contentHash).
#   2) Build the contentUri: <ContentUriBase>/win-server-WINRM-002.zip
